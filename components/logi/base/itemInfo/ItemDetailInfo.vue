<script lang="ts" setup>
import { VDataTable } from 'vuetify/labs/VDataTable'
import ItemDetailAddModal from '../itemInfo/modal/ItemDetailAddModal.vue'
import ItemDetailUpdateModal from './modal/ItemDetailUpdateModal.vue'
import { itemStore } from '@/store/logi/item'

const isAddDialogVisible = ref(false)
const isUpdateDialogVisible = ref(false)

const selectedItem: Ref<any[]> = ref([])
const selecteditem = ref([])

const codeheaders = [
  { title: '품목군코드', key: 'itemGroupCode' },
  { title: '품목군명', key: 'itemGroupName' },
  { title: '비고', key: 'description' },
]

const close = async () => {
  isAddDialogVisible.value = false
}

const cancel = async () => {
  isUpdateDialogVisible.value = false
}

const itemData = ref([])

// 품목군 조회
const search = async () => {
  await itemStore().GET_ITEMGROUP_INFO()
  itemData.value = itemStore().getItemGroupInfo
}

// 품목군 추가
const save = async (batchList: object) => {
  console.log('ruru', batchList)

  await itemStore().ADD_ITEMGROUP_INFO(batchList)
  itemData.value = itemStore().addItemGroupInfo

  close()

  search()
}

// 한 item 클릭 시 itemCode 추출
watch(selectedItem, (selectedValue: any) => {
  // console.log(selectedValue)
  // console.log(toRaw(selectedValue))
  // console.log(toRaw(selectedValue).length) // 1
  // console.log(toRaw(selectedValue[0]))
  // console.log(toRaw(selectedValue[0]))

  // console.log(toRaw(selectedValue[0].customerCode))
  if (toRaw(selectedValue.length) === 0)
    selecteditem.value = []
  else
    selecteditem.value = toRaw(selectedValue[0])
  console.log(selecteditem.value)
})

// 품목군 삭제
const deleteItemGroup = async () => {
  console.log('ㅅㅅㅅㅅ', selecteditem.value)
  console.log('xzzzz', selecteditem.value.itemGroupCode)

  if (!selecteditem.value.itemGroupCode) {
    console.error('No workplace selected for deletion')

    return
  }
  const userConfirmed = window.confirm('정말로 품목군을 삭제하시겠습니까?')

  if (userConfirmed) {
    try {
      const itemGroupCode = selecteditem.value.itemGroupCode

      // 삭제 API 호출
      await itemStore().DELETE_ITEMGROUP_INFO(itemGroupCode)

      // 삭제 후 다시 검색 등 필요한 작업 수행
      search()
    }
    catch (error) {
      console.error('데이터 삭제 오류:', error)
    }
  }
  else {
    console.log('품목군 삭제 취소됨')
  }
}
</script>

<template>
  <VCard>
    <div
      class="text-2xl"
      style="margin-left: 20px; margin-top: 20px"
    >
      <p> 품목군 관리</p>
    </div>

    <div style="margin-left: 20px; margin-top: 20px">
      <VBtn
        class="button"
        color="primary"
        @click="search"
      >
        품목군 조회
      </VBtn>
      <!--
        <VBtn
        class="button"
        color="primary"
        >
        추가
        </VBtn>
      -->
      <ItemDetailAddModal
        v-model="isAddDialogVisible"
        @close="close"
        @save="save"
      />
      <!--
        <VBtn
        class="button"
        color="primary"
        >
        수정
        </VBtn>
      -->
      <ItemDetailUpdateModal
        v-model="isUpdateDialogVisible"
        :selecteditem="selecteditem"
        @cancel="cancel"
        @update="update"
        @clear="search"
      />
      <VBtn
        class="button"
        color="primary"
        @click="deleteItemGroup"
      >
        품목군 삭제
      </VBtn>
    </div>
    <VCardText>
      <VDataTable
        v-model="selectedItem"
        :headers="codeheaders"
        :items="itemData"
        :items-per-page="5"
        show-select
        select-strategy="single"
        return-object
      />
    </VCardText>
  </VCard>
</template>
